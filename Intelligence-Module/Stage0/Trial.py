'''
Stage0: Compilation / Syntax Check
'''
import os
import subprocess
import tempfile
...
def prune_stderr(stderr: str) -> str:
    if not stderr:
        return ""
    ...
    for line in lines:
        l = line.lower().strip()
        if l.startswith("usage:") or l.startswith("use --help"):
            return ""
    ...
    for line in lines:
        ...
        line = re.sub(r'^(error|warning|note):\s*', '', line, flags=re.IGNORECASE)
...
def compile_test(code: str, language: str):
    try:
        ...
        pruned_error = prune_stderr(e.stderr)
        error_type = classify_from_stderr(pruned_error, language)
        return {
            "stage": 0,
            "status": "FAIL",
            "check_type": "compile_only",
        ...
        if language == "python":
            error_msg = str(e).split("(<submitted_code>")[0].strip()
